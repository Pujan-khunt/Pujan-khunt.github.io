function showNotesList(category) {
  ...
  // hide the categories
  const categoriesElement = document.querySelector('.side-bar-content');
  ...
  // Hide all the notes list and only show the notes of the selected category
  const allNotesList = notesListElement.querySelectorAll('ul');
  allNotesList.forEach(ul => ul.style.display = 'none');

  /// Display the notes of selected category
  const categoryNotes = document.getElementById(category);
  categoryNotes.style.display = 'flex';
  console.log(categoryNotes);
}

function loadNoteContent(notePath) {
  ...
  fetch(notePath)
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          `Could not fetch ${markdownFile}: ${response.statusText}`
        );
      }
